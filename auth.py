...
def obtain_fresh_token(file_name: str) -> Credentials:
...
def authenticate(token_json_fn: str = 'token.json',
                 authenticated_token_json_fn: str = 'authenticated.json') -> Credentials:
    ...
    if not os.path.exists(authenticated_token_json_fn):
        credentials = obtain_fresh_token(token_json_fn)
